...
def main(model_fix, ckpt):
    ...
    tf_train = JointTransform2D(img_size=args.encoder_input_size, low_img_size=args.low_image_size,
                                ori_size=opt.img_size, crop=opt.crop, p_flip=0.5, p_rota=0.5, p_scale=0.0, p_gaussn=0.0,
    ...
    for fold in range(kfold):
        ...
        model.to(device)
        print(model.image_encoder.cnn_embed)
        ...
        criterion = get_criterion(args.modelname,opt)
        ...
        for epoch in range(opt.epochs):
            ...
            if (epoch + 1) >= 17:
                ...
                for batch_idx, (datapack) in enumerate(valloader):
                    ...
                    val_loss = dc(pred, label)
                    val_loss_dice.append(val_loss.detach().cpu().numpy())
                val_loss_dice_mean = np.mean(val_loss_dice)

                if val_loss_dice_mean <= best_score or (epoch == 29):
                    best_score = val_loss_dice_mean
                    if not os.path.isdir(opt.save_path):
                        os.makedirs(opt.save_path)
                    save_path = f'./checkpoints/{model_fix}/' + f'fold{fold}/' + args.modelname + '_' + 'fold' + str(
                        fold) + '_' + 'epoch' + str(
                        epoch) + '_' + str(best_score)

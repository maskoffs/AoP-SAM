...
class EvalDice(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        dice = self.dc(F.one_hot(torch.argmax(low_res_logits_t, dim=1), 3), target_t)
        return dice
...
def main(model_fix):
    ...
    for fold in range(kfold):
        ...
        ckpt_file = os.listdir(ckpt_path)[-1]  # weight
        ...
        parser.add_argument('--base_lr', type=float, default=0.0001)  # 0.0006
        ...
        tf_train = JointTransform2D(img_size=args.encoder_input_size, low_img_size=args.low_image_size,
                                    ...
                                    p_gaussn=0.0,p_contr=0.0, p_gama=0.0, p_distor=0.0, color_jitter_params=None,
        ...
        # ====================  first evaluation to get the dice loss after loading pretrained weight file ============ #
        model.eval()
        val_loss_dice = []
        evaldc = EvalDice()
        ...
        print(f"First Test on Test Set: {test_score}")

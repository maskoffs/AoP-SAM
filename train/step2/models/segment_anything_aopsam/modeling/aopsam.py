...
class AoPSAM(nn.Module):
    ...
    def __init__(
        ...
        for n, value in self.image_encoder.named_parameters():
            if "cnn_embed" not in n and "pos_embed_proj" not in n and "CBAtt" not in n and "Adapter" not in n and "2.attn.rel_pos" not in n and "5.attn.rel_pos" not in n and "8.attn.rel_pos" not in n and "11.attn.rel_pos" not in n and "neck" not in n:
                value.requires_grad = False
            else:
                value.requires_grad = True
                if 'conv_branch' in n or 'cat_conv_branch' in n:
    ...
    def forward(
        ...
        # print(f"forward Step\n img shape: {imgs.shape}, masks shape: {masks.shape}")
        imge = self.image_encoder(imgs)
        # print(f"Image Embedding shape: {imge.shape}")

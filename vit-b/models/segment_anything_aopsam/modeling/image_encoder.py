...
import math
from cnn_embedding_modules.Muti_Scale.MSBranch import MutiScale_Conv_Brach
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.cnn_embed = MutiScale_Conv_Brach()
        self.patch_embed = NewPatchEmbed(
            in_chans=3,
            embed_dim=embed_dim,
        )
        self.pos_embed: Optional[nn.Parameter] = None
        if use_abs_pos:
            self.pos_embed = nn.Parameter(torch.zeros(1, 1024 // 16, 1024 // 16, embed_dim))
            self.pos_embed_proj = PosEmbedProj(embed_dim=embed_dim, ori_feature_size=1024 // 16,
...
class ParaBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.window_size == 0:
            self.CBAtt = CBAtt(dim=dim, num_heads=num_heads)
            self.cnn_Adapter = CnnAdapter(in_channels=dim, out_channels=dim)
    ...
    def forward(self, x: torch.Tensor, cnnx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.window_size == 0:
            cbax = self.CBAtt(x, cnnx, cnnx)  # b h w c
            x = x + cbax
            cnnx = self.cnn_Adapter(cnnx.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        ...
        if self.window_size == 0:
            x = x + self.param * self.mlp_Adapter(xn)
...
class NewPatchEmbed(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=16, stride=(8, 8), padding=padding)
...
class PosEmbedProj(nn.Module):
    def __init__(
        ...
        for i in range(downtimes):
            down = DownSampleBlock(embed_dim, embed_dim)
